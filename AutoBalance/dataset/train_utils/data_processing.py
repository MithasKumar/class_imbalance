...
def get_raw_data_to_loaders(configs):
	...
	configs['Source_Eval_Loader'],configs['Source_Eval_Negatives'],configs['Source_Eval_Positives'] = get_raw_df_to_loader(data=configs['Source_eval_df'],configs=configs['Source_configs'],is_lbls=True,tseq_len=configs['adap_len'],blen=configs['batch_size'],norm_dict=norm_dict,ps_lbls_dict=psl_dict_sr)
	print("Making Source Test Loader --> ")
	configs['Source_Test_Loader'],configs['Source_Test_Negatives'],configs['Source_Test_Positives'] = get_raw_df_to_loader(data=configs['Source_test_df'],configs=configs['Source_configs'],is_lbls=True,tseq_len=configs['adap_len'],blen=configs['batch_size'],norm_dict=norm_dict,ps_lbls_dict=psl_dict_sr)
	print("")
	print("Making Target Train Loader --> ")
	configs['Target_Train_Loader'],configs['Target_Train_Negatives'],configs['Target_Train_Positives'] = get_raw_df_to_loader(data=configs['Target_train_df'],configs=configs['Target_configs'],is_lbls=True,to_shfl=True,tseq_len=configs['adap_len'],blen=configs['batch_size'],is_src=False,norm_dict=norm_dict,ps_lbls_dict=psl_dict_sr,subset_dict=configs["subset_dict"])
	print("Making Target Eval Loader --> ")
	configs['Target_Eval_Loader'],configs['Target_Eval_Negatives'],configs['Target_Eval_Positives'] = get_raw_df_to_loader(data=configs['Target_eval_df'],configs=configs['Target_configs'],is_lbls=True,tseq_len=configs['adap_len'],blen=configs['batch_size'],is_src=False,norm_dict=norm_dict,ps_lbls_dict=psl_dict_sr)
	print("Making Target Test Loader --> ")
	configs['Target_Test_Loader'],configs['Target_Test_Negatives'],configs['Target_Test_Positives'] = get_raw_df_to_loader(data=configs['Target_test_df'],configs=configs['Target_configs'],is_lbls=True,tseq_len=configs['adap_len'],blen=configs['batch_size'],is_src=False,norm_dict=norm_dict,ps_lbls_dict=psl_dict_sr)
