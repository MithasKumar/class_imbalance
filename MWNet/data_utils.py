...
#0 is good, and 1 is bad wheel
def prepare_meta_data(configs, meta_ex_per_class):
    #train_loader = configs['Source_Train_Loader']
    meta_data_loader = DataLoader(dataset=copy.deepcopy(configs['Source_Train_Dataset']), batch_size=configs['batch_size'], shuffle=True)
   # meta_data_loader = copy.deepcopy(train_loader)
    all_indices = np.arange(len(meta_data_loader.dataset.tensors[0]))
    ones_indices = []
    for i,tensor in enumerate(meta_data_loader.dataset.tensors[1]):
        if tensor == 1:
            #print(i)
            ones_indices.append(i)
    
    zero_indices = list(set(all_indices) - set(ones_indices))
    random.shuffle(ones_indices)
    random.shuffle(zero_indices)
   # print(len(zero_indices))
    meta_data_loader.dataset.tensors = list(meta_data_loader.dataset.tensors)
    #train_loader.dataset.tensors = list(train_loader.dataset.tensors)

    #print(len(ones_indices[:meta_ex_per_class]))
    #print(len(zero_indices[:meta_ex_per_class]))
    indices_to_keep = ones_indices[:meta_ex_per_class] + zero_indices[:meta_ex_per_class]
    #print(indices_to_keep)
    #exit()
    for i in range(4):
        meta_data_loader.dataset.tensors[i] = meta_data_loader.dataset.tensors[i][indices_to_keep]

    meta_data_loader.dataset.tensors = tuple(meta_data_loader.dataset.tensors)
    #print(all_indices)
    train_loader = configs['Source_Train_Loader']
    train_loader.dataset.tensors = list(train_loader.dataset.tensors)
    train_ind_to_keep = list(set(all_indices) - set(indices_to_keep))
    #print(len(train_ind_to_keep))
    for i in range(4):
        train_loader.dataset.tensors[i] = train_loader.dataset.tensors[i][train_ind_to_keep]

    train_loader.dataset.tensors = tuple(train_loader.dataset.tensors)

    return meta_data_loader, train_loader
