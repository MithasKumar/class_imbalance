...
def rand_seed_devices(configs):    #Random seed to all devices
...
class MLP_model(MetaModule):
    def __init__(self,f1,f2,f3,ld):
        ...
        self.fc1_lin = MetaLinear(self.latent_dim,self.f1)
        self.fc1_dropout = nn.Dropout(0.25)
        self.fc2_lin = MetaLinear(self.f1,self.f2)
        self.fc2_dropout = nn.Dropout(0.5)
        self.fc3_lin = MetaLinear(self.f2,self.f3)
        self.fc3_dropout = nn.Dropout(0.5)
        self.fc4 = MetaLinear(self.f3,2)
    ...
    def forward(self, x):
        ...
        out = self.fc2_lin(out)
        out = self.relu_act(out)
        out = self.fc2_dropout(out)
        out = self.fc3_lin(out)
        out = self.relu_act(out)
